...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-white shadow-md z-50 px-6 py-4 font-['Neue Montreal'] flex justify-between items-center">
      {/* Logo */}
      <div className="logo">
        <svg
          width="72"
          ...
          className="text-black" // ensures fill="currentColor" uses black
        >
          <path
        ...
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map(
          (item, index) => (
            <a
              ...
              className={`text-md capitalize font-light hover:text-blue-600 ${
                index === 4 ? "ml-8" : ""
              }`}
            >
              {item}
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="absolute top-full left-0 w-full bg-white shadow-md flex flex-col items-center md:hidden py-4 gap-4 z-40">
          {["Services", "Our Work", "About Us", "Insights", "Contact"].map(
            (item, index) => (
              <a
                key={index}
                href={`#${item.toLowerCase().replace(" ", "")}`}
                className="text-md capitalize font-light text-black hover:text-blue-600"
